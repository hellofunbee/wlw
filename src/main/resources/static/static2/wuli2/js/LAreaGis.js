...
window.LArea = (function() {
    ...
    MobileArea.prototype = {
        ...
        finish: function(e) {
            ...
            //新加的交互
           if($('#li4').parent('.menu_e_list').hasClass('choose')){//预警
               $('.search_l_more').attr('data-more',2)
               .attr('data-pro',provinceCode)
               .attr('data-city',(cityCode)?(cityCode):(''))
				.attr('data-qu',((countyCode)?(countyCode):('')));
				$('.search_menus>li').eq(0).click();
            }else if($('#li3').parent('.menu_e_list').hasClass('choose')||$('#li2').parent('.menu_e_list').hasClass('choose')){//分布图
            	$.ajax({
					...
					data: JSON.stringify({
						...
						"d_province": provinceCode,
						"d_city":((cityCode)?(cityCode):('')),
						"d_district":((countyCode)?(countyCode):('')),
					}),
						...
						if(data.state == 0) {
							if(data.object!=null){
								...
								$('.speciesMap .uu_map>li').on('click', function() {
									...
									if($('#li2').parent('.menu_e_list').hasClass('choose')){
										b_createMap()
										getBoundary(index_name, $(this).index(), "作物种植面积比例",1);
									}else if($('#li3').parent('.menu_e_list').hasClass('choose')){
										b_createMap()
								...
								$(".speciesMap .uu_map").html('<span class="noNodata">暂无数据</span>');
								$('.toRight').hide();
								$('#allmap').hide();
								$('#nothmap').show();
							}
