...
;!function () {
    ...
    C.prototype = [], C.prototype.constructor = C, w.extend = function () {
        ...
        try{
        if (!e.format || e.format == 'yyyy-MM-dd') {


            if (e.elem)
                if (e.elem.indexOf('_from') != -1) {
                    e.value = UI.getCData(-1).format('yyyy-MM-dd')
                } else if (e.elem.indexOf('_to') != -1) {
                    e.value = UI.getCData(0).format('yyyy-MM-dd')
                } else if (e.elem.indexOf('ds') != -1) {
                    e.value = UI.getCData(-1).format('yyyy-MM-dd')
                } else if (e.elem.indexOf('de') != -1) {
                    e.value = UI.getCData(0).format('yyyy-MM-dd')
                } else if (e.elem.indexOf('ts') != -1) {
                    e.value = UI.getCData(-1).format('yyyy-MM-dd')
                } else if (e.elem.indexOf('te') != -1) {
                    e.value = UI.getCData(0).format('yyyy-MM-dd')
                }

            // e.value = UI.getCData(0).format('yyyy-MM-dd')
        } else if (e.format == 'yyyy-MM-dd hh:mm:ss') {
            e.value = UI.getCData(0).format('yyyy-MM-dd hh:mm:ss')
        }
        }catch (e){
            console.log(e)
        }
