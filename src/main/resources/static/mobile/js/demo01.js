...
$(function () {
	...
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin(786, 576, {
        iWndowType: 1,
...
function clickGetAudioInfo() {
	$('.speakText', window.parent.document).text('获取中..')
	var szIP = $("#ip").val();
	...
	WebVideoCtrl.I_GetAudioInfo(szIP, {
		success: function (xmlDoc) {
			...
			$('.speakText', window.parent.document).text('通道成功')
			clickStartVoiceTalk()
		},
		error: function () {
			showOPInfo(szIP + " 获取对讲通道失败！");
			$('.speakText', window.parent.document).text('通道失败').attr('data-s',0)
			alert('获取对讲通道失败！')
		}
	});
}
...
function clickStartVoiceTalk() {
	...
	if (0 == iRet) {
		szInfo = "开始对讲成功！";
		$('.speakText', window.parent.document).text('结束对讲').attr('data-s',1)
	} else {
		szInfo = "开始对讲失败！";
		alert("开始对讲失败！")
		$('.speakText', window.parent.document).text('对讲').attr('data-s',0)
	}
	showOPInfo(szIP + " " + szInfo);
}
