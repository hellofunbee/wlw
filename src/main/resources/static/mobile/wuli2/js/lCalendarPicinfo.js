	...
	MobileCalendar.prototype = {
		...
		bindEvent: function(type) {
			...
			function finishMobileDate(e) {
				...
				var houtai  =  (date_yy % passY + _self.minY) + "-" + date_mm + "-" + date_dd
				var new_data = (date_yy % passY + _self.minY) + "/" + date_mm + "/" + date_dd
				_self.trigger.value = new_data;
				//新加后台交互
				listIPCPointIMG(hasTime(),1,houtai)
				closeMobileCalendar(e);
			}
		//新加后台交互	
			function hasTime(){
				var state;
				$('.pic_co_s_i_eye ').each(function() {
					if($(this).hasClass('activeJian')) {
						state = $(this)
					}
				});
				return state.attr("data-monitorid");
			}
