...
import com.jingu.IOT.dao.UserQuestionDao;
...
public class UserQuestionService {
    ...
    public IOTResult save(PageData pd) {
        int back = userQuestionDao.save(pd);
    ...
    public IOTResult del(PageData pd) {
        int back = userQuestionDao.del(pd);
        if (back > 0)
    ...
    public IOTResult update(PageData pd) {
        int back = userQuestionDao.update(pd);
        if (back > 0)
    ...
    public IOTResult list(PageData pd) {
        List<Map<String, Object>> back = userQuestionDao.list(pd);
